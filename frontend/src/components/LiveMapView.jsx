...
const LiveMapView = () => {
  ...
  }, [isSharing, user?.room]);  // Added isSharing to dependencies

  // Update location in backend and notify room
  const updateLocation = async (lat, lng) => {
    if (!socketRef.current || !user?.room) return;
    
    try {
      // Update backend
      await axios.patch('http://localhost:4000/api/auth/me', {
        currentLocation: isSharing ? { latitude: lat, longitude: lng } : null
      }, {
  ...
  const fetchRoomMembers = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:4000/api/rooms/members', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      setRoomMembers(response.data.filter(member => member._id !== user._id));
    } catch (error) {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const shouldShare = window.confirm('Share your live location with room members?');
      setIsSharing(shouldShare);
    } catch (error) {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:4000/api/rooms/${roomIdInput}/join`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleLeaveRoom = async () => {
    ...
    setSuccess(null);
    try {
      const response = await axios.post('http://localhost:4000/api/rooms/leave', {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      ...
      // Notify room members
      if (socketRef.current && user.room) {
  ...
  const toggleSharing = async () => {
    ...
    try {
      ...
      // Update backend
      await axios.patch('http://localhost:4000/api/auth/me', {
        currentLocation: newSharingState ? {
          latitude: position[0],
          longitude: position[1]
        } : null,
        isSharing: newSharingState  // Add this to track sharing state in backend
      }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      ...
      setIsSharing(!newSharingState); // Revert state on error
      setError('Failed to update sharing status. Please try again.');
    }
  };
  ...
  useEffect(() => {
    const fetchSharingState = async () => {
      try {
        ...
        setIsSharing(response.data.isSharing || false);
      } catch (error) {
        console.error('Failed to fetch sharing state:', error);
      }
    };
  ...
  return (
    <div style={{ margin: 0, height: '95vh', width: '100vw', position: 'fixed', overflow: 'hidden' }}>
      <MapContainer
        center={position || [0, 0]}
        zoom={15}
